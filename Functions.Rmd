...
# data = read.csv("/Users/huangyt_tina/Desktop/Capstone_Lakdawala Lab/Flying-Ferrets/csv files/Brown.Final.cleaned.csv")
# data = read.csv("/Users/xuxiaonuo/Desktop/36726_statistical_practice/project/code/Flying-Ferrets/csv files/Brown.Final.cleaned.csv")

data = read.csv("cleaned_penwalls_only.csv")
...
result1 <- filter_function(split_data, "Donor", "Touching", "R1", "Touching", "Panel O")
...
result <- duration_function(filtered_data, subject_1 = "Donor", subject_1_behave = "Touching", subject_2 = "R1", subject_2_behave = "Touching", modifier = "Panel O")
...
```{r}
result_reordered <- organize_result(result, subject_1 = "Donor", subject_1_behave = "Touching", subject_2 = "R1", subject_2_behave = "Touching", modifier = "Panel O")
...
sum(score_data[score_data$Subject == "R1" & score_data$Synchronous == F, "score"])

nrow(score_data[which(score_data$Synchronous == T), ])

sum(score_data[score_data$Subject == "R1" & score_data$Synchronous== T, "score"])
```
    ...
    for (behavior_2 in unique_behaviors) {
      for (modifier in unique_modifiers) {
        for (subject_2 in unique_subjects) {
          ...
          #if(subject_2 == "R1" & modifier == "Panel O"){
          #  print(nrow(score_data[which(score_data$Synchronous == F), ]))
          #  return(duration_data)
          #  print(nrow(score_data[which(score_data$Synchronous == T), ]))
          #}
...
summary_df_reorganized <- summary_df_r1 %>% 
  full_join(summary_df_r2, by = c("Behavior_Subject1", "Behavior_Subject2", "Modifier", "Synchronous")) %>%
  full_join(summary_df_r3, by = c("Behavior_Subject1", "Behavior_Subject2", "Modifier", "Synchronous")) %>%
  full_join(summary_df_r4, by = c("Behavior_Subject1", "Behavior_Subject2", "Modifier", "Synchronous"))
...
final_df_score <- summary_df_reorganized %>%
  ...
  select(Behavior_Subject1, Behavior_Subject2, Modifier, Synchronous, Score.x, Score.y, Score.x.x, Score.y.y)  %>% 
...
final_df_max_score<- summary_df_reorganized %>%
  ...
  select(Behavior_Subject1, Behavior_Subject2, Modifier, Synchronous, Max_Score.x, Max_Score.y, Max_Score.x.x, Max_Score.y.y)  %>% 
...
final_df_frequency <- summary_df_reorganized %>%
  ...
  select(Behavior_Subject1, Behavior_Subject2, Modifier, Synchronous, Frequency.x, Frequency.y, Frequency.x.x, Frequency.y.y)  %>% 
  # Rename the columns of scores as "R1", "R2", "R3", R4"
