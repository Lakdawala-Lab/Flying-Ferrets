    ...
    for (behavior_2 in unique_behaviors) {
      for (modifier in unique_modifiers) {
        for (subject_2 in unique_subjects) {
          ...
          summary_row <- data.frame(
            Subject = subject_2,
            Behavior = paste(subject_1, behavior_1, modifier, behavior_2),
            Synchronous = F,
            Score = sum(score_data[score_data$Subject == subject_2 & score_data$Synchronous == F, "score"]),
            Duration = sum(score[which(score_data$Synchronous == F), "e"]),
            Frequency = nrow(score[which(score_data$Synchronous == F), ])
          )
          ...
          summary_row_synchronous <- data.frame(
            ...
            Synchronous = T,
            Score = sum(score_data[score_data$Subject == subject_2 & score_data$Synchronous == T, "score"]),
            Duration = sum(score[which(score_data$Synchronous == T), "e"]),
            Frequency = nrow(score[which(score_data$Synchronous == T), ])
          )
...
summary_df_reorganized <- summary_df_r1 %>% 
  full_join(summary_df_r2, by = join_by(Behavior, Synchronous)) %>%
  full_join(summary_df_r3, by = join_by(Behavior, Synchronous)) %>%
  full_join(summary_df_r4, by = join_by(Behavior, Synchronous))
...
final_df_score <- summary_df_reorganized %>%
  ...
  select(Behavior, Score.x, Score.y, Score.x.x, Score.y.y)  %>% 
  # Rename the columns of scores as "R1", "R2", "R3", R4"
  rename(R1 = Score.x, R2 = Score.y, R3 = Score.x.x, R4 = Score.y.y)

# Create the final table of duration
final_df_duration <- summary_df_reorganized %>%
  # Select the duration columns
  select(Behavior, Duration.x, Duration.y, Duration.x.x, Duration.y.y)  %>% 
  # Rename the columns of duration as "R1", "R2", "R3", R4"
  rename(R1 = Duration.x, R2 = Duration.y, R3 = Duration.x.x, R4 = Duration.y.y)
...
final_df_frequency <- summary_df_reorganized %>%
  ...
  select(Behavior, Frequency.x, Frequency.y, Frequency.x.x, Frequency.y.y)  %>% 
  # Rename the columns of scores as "R1", "R2", "R3", R4"
